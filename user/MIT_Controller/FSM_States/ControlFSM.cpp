                          ...
                          MIT_UserParameters* userParameters)
{
  ...
  statesList.bypass = new FSM_State_Bypass<T>(&data);
...
void ControlFSM<T>::runFSM() {
  ...
  if(data.controlParameters->use_rc){
    ...
    } else if(rc_mode == RC_mode::BYPASS){
      data.controlParameters->control_mode = K_BYPASS;
    } 
...
FSM_State<T>* ControlFSM<T>::getNextState(FSM_StateName stateName) {
  ...
  switch (stateName) {
    ...
    case FSM_StateName::BYPASS:
      return statesList.bypass;
