...
EdgeTrigger<TaranisSwitchState> bypass_prep_edge_trigger(SWITCH_UP);
...
void sbus_packet_complete() {
  ...
  switch(estop_switch) {
    ...
    case SWITCH_DOWN: // run 
      ...
      // Experiment mode (two leg stance, vision, ...)
      if(bypass_prep_edge_trigger.trigger(mode_selection_switch) 
          && mode_selection_switch == SWITCH_UP) {
        initial_mode_go_switch = mode_go_switch;
      }
      ...
      else if(mode_selection_switch == SWITCH_UP){
        selected_mode = RC_mode::BYPASS;
        //printf("test!\n");
      }
